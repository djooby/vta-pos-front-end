...
export default function SubCategories({
  ...
  const [subCategorys, setSubCategorys] = useState<Demo.SubCategory[]>([]);
    ...
    try {
      await axios.post("/api/category/sub_category", dataToapi).then((res) => {
        ...
        if (result.status === "success") {
          ...
          setSubCategorys(result.data.sub_category);
        } else {
  ...
  const deleteSubCategory = async () => {
    ...
    try {
      await axios.post("/api/subCategory/delete", dataToApi).then((res) => {
        const result = res.data;
        if (result.status === "success") {
  ...
  const onRowEditComplete = async (e: any) => {
    setLoading(true);
    let _subCategory = [...subCategorys];
    let { newData, index } = e;
    _subCategory[index] = newData;

    //? modifier le produit
    const dataToApi = {
      token: userInfo.token,
      new_subCategory: newData,
    };

    try {
      await axios.post("/api/subCategory/update", dataToApi).then((res) => {
        const result = res.data;
        if (result.status === "success") {
          toastMessage("success", result.data);
  ...
  useEffect(() => {
    ...
    initFilters();
  }, [getSubCategories, layoutConfig, userInfo]);
